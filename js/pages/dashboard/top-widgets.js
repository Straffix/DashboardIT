(function initializeDashboardTopWidgetsModule() {
	...
	dashboardModules.createTopWidgetsController = ({ elements, services, controllers } = {}) => {
		...
		const saveLayout = (layout = state.layout) => {
			...
			preferencesService?.saveDashboardTopWidgets?.(normalizedLayout) || storageService?.writeJson?.(topWidgetsStorageKey, normalizedLayout)
		}

		const normalizeQuickNote = value => String(value ?? '').replace(/\r\n/g, '\n').slice(0, 220)
		...
		const getUserDisplayName = authorId => {
			const matchedUser = (usersService?.getAll?.() || []).find(user => String(user?.id || '') === String(authorId || ''))
			return String(matchedUser?.fullName || '').trim() || 'Zespol'
		}

		const loadTasks = () => {
			const rawTasks = preferencesService?.getDashboardTasks?.() || storageService?.readJson?.(tasksStorageKey, []) || []
			...
			return rawTasks
				...
				.filter(Boolean)
				.sort((leftTask, rightTask) => `${leftTask.date}T${leftTask.time}`.localeCompare(`${rightTask.date}T${rightTask.time}`, 'pl'))
		}
		...
		const getNotesWidgetSnapshot = () => {
			...
			const messages = Array.isArray(notes)
				? notes
						...
						.sort((leftMessage, rightMessage) => (Date.parse(rightMessage.updatedAt || rightMessage.createdAt) || 0) - (Date.parse(leftMessage.updatedAt || leftMessage.createdAt) || 0))
				: []
		...
		const createClockWidgetElement = () => {
			const element = createElementFromHtml(`
				<article
					...
					aria-label="Zegar">
					<strong class="dashboard-top-mini-widget-value dashboard-top-clock-time">--:--</strong>
			...
			if (valueElement && metaElement) {
				dynamicRefs.clock.push({ valueElement, metaElement })
			}
		...
		const createTasksWidgetElement = () =>
			createElementFromHtml(
				createCustomWidgetMarkup('tasks', getTasksWidgetSnapshot(), {
					action: 'planner',
					icon: 'check-solid-full',
				})
			)
					...
					<article
						...
						<div class="dashboard-top-notes-field">
							<textarea
								...
								spellcheck="false"
								aria-label="Szybka notatka"
								placeholder="Kliknij i wpisz szybka notatke...">${escapeHtml(noteValue)}</textarea>
						</div>
					</article>
				`)
		...
		const refreshDynamicWidgets = () => {
			const now = new Date()
			const timeLabel = `${padNumber(now.getHours())}:${padNumber(now.getMinutes())}`
			...
			dynamicRefs.clock.forEach(ref => {
				ref.valueElement.textContent = timeLabel
		...
		const init = () => {
			...
			dashboardTopWidgetPicker.addEventListener('click', event => {
				...
				updateLayout(optionButton.getAttribute('data-widget-slot') || '', optionButton.getAttribute('data-widget-id') || '')
			})

			dashboardTopWidgetLayout.addEventListener('click', handleWidgetAction)
