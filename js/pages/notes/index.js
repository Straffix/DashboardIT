document.addEventListener('DOMContentLoaded', () => {
	...
	function getRoleLabel(role) {
		return AppUtils.auth.getRoleLabel?.(role) || (role === 'admin' ? 'Lider' : 'Członek')
	}
	...
	function getActorMeta(userId) {
		...
		const displayName = matchedUser?.fullName || 'Nieznany użytkownik'
		...
		if (currentUser) {
			...
			authText.textContent =
				currentUser.role === 'admin'
					? 'Możesz zarządzać notatkami, ważnymi tematami i przypisywać zadania do członków zespołu.'
					: 'Możesz dodawać wpisy, edytować swoje rekordy i zmieniać status zadań przypisanych do Ciebie.'
			authBtn.innerHTML = '<i class="fa-solid fa-user-gear"></i><span>Otwórz profil</span>'
			return
		}

		authCallout.classList.remove('is-active-user')
		authTitle.textContent = 'Podgląd tablicy jest dostępny dla wszystkich'
		authText.textContent =
			'Zaloguj się, aby dodawać wpisy, a jako lider również przypisywać zadania do innych osób.'
	...
	function renderBoardSummary(announcements, notes, tasks, currentUser) {
		...
		if (totalEntries === 0) {
			daySummary.textContent = currentUser
				? 'Tablica jest jeszcze pusta. Dodaj pierwszy ważny temat, notatkę albo zadanie dla zespołu.'
				: 'Tablica jest jeszcze pusta. Zaloguj się, aby dodać pierwszy wpis lub zadanie dla zespołu.'
			return
		}

		daySummary.textContent = `Na tablicy jest ${announcements.length} ważnych tematów, ${notes.length} notatek i ${tasks.length} zadań. Ostatnia zmiana: ${latestUpdate}.`
	}
	...
	function renderUserStatus(currentUser) {
		if (!currentUser) {
			userStatusBox.innerHTML = `
				<strong>Gość</strong>
				<p>Podgląd tablicy bez możliwości dodawania, przypisywania i edycji rekordów.</p>
			`
			return
		}

		const description = currentUser.role === 'admin'
			? 'Lider może zarządzać wszystkimi wpisami i przypisywać zadania innym użytkownikom.'
			: 'Członek zespołu może dodawać swoje wpisy i aktualizować status zadań przypisanych do siebie.'
	...
	function renderTaskAdminCallout(currentUser) {
		...
		if (!currentUser) {
			taskAdminTitle.textContent = 'Zaloguj się jako lider, aby przypisywać zadania'
			taskAdminText.textContent =
				'Każdy może podejrzeć listę zadań, ale ich tworzenie i przypisywanie do ludzi jest dostępne tylko dla lidera.'
			return
		}
		...
		if (currentUser.role === 'admin') {
			...
			taskAdminText.textContent =
				'Możesz tworzyć zadania, przypisywać je do użytkowników, ustawiać priorytet oraz edytować całą tablicę zadań.'
			return
		}
		...
		taskAdminText.textContent =
			'Widzisz całą listę zadań. Jeśli jakieś zadanie jest przypisane do Ciebie, możesz samodzielnie zaktualizować jego status.'
	}
	...
	function populateTaskAssigneeOptions(users, selectedUserId = '') {
		...
		if (users.length === 0) {
			taskAssigneeSelect.innerHTML = '<option value="">Brak aktywnych użytkowników</option>'
			return
		}
	...
	function updateAnnouncementComposer(currentUser) {
		...
		announcementSubmitBtn.textContent = state.editingAnnouncementId ? 'Zapisz ważny temat' : 'Opublikuj temat'
		announcementCancelBtn.hidden = !currentUser || !state.announcementComposerOpen

		if (!currentUser) {
			announcementToggleBtn.innerHTML = '<i class="fa-solid fa-right-to-bracket"></i><span>Zaloguj się, aby dodać</span>'
			return
		}

		if (state.editingAnnouncementId) {
			announcementToggleBtn.innerHTML = '<i class="fa-solid fa-pen-to-square"></i><span>Edytujesz ważny temat</span>'
			return
		}
		...
		announcementToggleBtn.innerHTML = state.announcementComposerOpen
			...
			: '<i class="fa-solid fa-bullhorn"></i><span>Dodaj ważny temat</span>'
	}
	...
	function updateNoteComposer(currentUser) {
		...
		if (!currentUser) {
			noteToggleBtn.innerHTML = '<i class="fa-solid fa-right-to-bracket"></i><span>Zaloguj się, aby dodać</span>'
			return
		}

		if (state.editingNoteId) {
			noteToggleBtn.innerHTML = '<i class="fa-solid fa-pen-to-square"></i><span>Edytujesz notatkę</span>'
			return
		}

		noteToggleBtn.innerHTML = state.noteComposerOpen
			? '<i class="fa-solid fa-chevron-up"></i><span>Schowaj formularz</span>'
			: '<i class="fa-solid fa-file-lines"></i><span>Dodaj notatkę</span>'
	}
	...
	function updateTaskComposer(currentUser) {
		...
		if (!currentUser) {
			taskToggleBtn.innerHTML = '<i class="fa-solid fa-right-to-bracket"></i><span>Zaloguj się, aby dodać</span>'
			return
		}

		if (!canManageTasks(currentUser)) {
			taskToggleBtn.innerHTML = '<i class="fa-solid fa-lock"></i><span>Zadania tylko dla lidera</span>'
			return
		}

		if (state.taskComposerOpen && !hasAssignableUsers) {
			taskFormHint.hidden = false
			taskFormHint.textContent = 'Brakuje aktywnych kont w systemie. Dodaj lub przywróć użytkownika, zanim przypiszesz zadanie.'
		}
	...
	function openTaskComposer(task = null) {
		...
		if (!canManageTasks(currentUser)) {
			showFeedbackMessage('Tylko lider może tworzyć i przypisywać zadania.', 'info')
			return
		}
	...
	function createEntryActionsMarkup({ type, entryId, canManage }) {
		...
		return `
			<div class="notes-entry-actions">
				...
				<button type="button" class="notes-icon-btn is-danger" data-entry-type="${type}" data-entry-action="delete" data-entry-id="${entryId}" aria-label="Usuń wpis">
		...
		const topActions = canManageTasks(currentUser)
			? `
				<div class="notes-entry-actions">
					...
					<button type="button" class="notes-icon-btn is-danger" data-task-action="delete" data-task-id="${task.id}" aria-label="Usuń zadanie">
						<i class="fa-solid fa-trash-can"></i>
					</button>
				</div>
			`
	...
	function renderAnnouncements(currentUser, announcements) {
		if (announcements.length === 0) {
			announcementsList.innerHTML = createEmptyStateMarkup({
				title: 'Brak ważnych tematów',
				copy: currentUser
					? 'Dodaj pierwszy przypięty komunikat dla zespołu i trzymaj najważniejsze sprawy na górze.'
					: 'Po zalogowaniu można dodać pierwszy przypięty komunikat dla zespołu.',
				accent: 'announcement',
			})
			return
		}
					...
					<article class="notes-entry-card is-announcement">
						<div class="notes-entry-top">
							<div class="notes-entry-tags">
								<span class="notes-entry-tag is-pinned">Ważny temat</span>
								<span class="notes-entry-tag">Przypięty</span>
							</div>
							${actionMarkup}
						</div>
	...
	function renderNotes(currentUser, notes) {
		if (notes.length === 0) {
			notesList.innerHTML = createEmptyStateMarkup({
				title: 'Brak wspólnych notatek',
				copy: currentUser
					? 'Dodaj pierwszy tekst, checklistę albo szybką instrukcję dla pozostałych osób.'
					: 'Po zalogowaniu można zapisać pierwszą wspólną notatkę dla zespołu.',
			})
			return
		}
			...
			.map(entry => {
				...
				return `
					<article class="notes-entry-card">
						<div class="notes-entry-top">
							<div class="notes-entry-tags">
								<span class="notes-entry-tag is-muted">Wspólna notatka</span>
							</div>
							${actionMarkup}
						</div>
	...
	function renderTasks(currentUser, tasks) {
		if (tasks.length === 0) {
			tasksList.innerHTML = createEmptyStateMarkup({
				title: 'Brak zadań zespołowych',
				copy: currentUser?.role === 'admin'
					? 'Dodaj pierwsze zadanie i przypisz je do jednej z osób w zespole.'
					: 'Lista zadań pojawi się tutaj, gdy lider przypisze pierwsze zadanie.',
				accent: 'task',
			})
			return
		}
	...
	announcementForm.addEventListener('submit', event => {
		...
		try {
			if (state.editingAnnouncementId) {
				...
				showFeedbackMessage('Zapisano zmiany w ważnym temacie.', 'success')
			} else {
				...
				showFeedbackMessage('Dodano nowy ważny temat dla zespołu.', 'success')
			}

			closeAnnouncementComposer()
			refreshView()
		} catch (error) {
			showFeedbackMessage(error.message || 'Nie udało się zapisać ważnego tematu.', 'error')
		}
	})
	...
	noteForm.addEventListener('submit', event => {
		...
		try {
			if (state.editingNoteId) {
				...
				showFeedbackMessage('Dodano nową notatkę do wspólnej tablicy.', 'success')
			}

			closeNoteComposer()
			refreshView()
		} catch (error) {
			showFeedbackMessage(error.message || 'Nie udało się zapisać notatki.', 'error')
		}
	})
	...
	taskForm.addEventListener('submit', event => {
		...
		if (!canManageTasks(currentUser)) {
			showFeedbackMessage('Tylko lider może tworzyć i przypisywać zadania.', 'error')
			return
		}

		if (taskSubmitBtn.disabled || !taskAssigneeSelect.value) {
			showFeedbackMessage('Brakuje aktywnego użytkownika do przypisania zadania.', 'error')
			return
		}
		...
		try {
			...
			showFeedbackMessage(error.message || 'Nie udało się zapisać zadania.', 'error')
		}
	})
	...
	async function handleEntryAction(event) {
		...
		if (!canManageEntry(entry, currentUser)) {
			showFeedbackMessage('Tylko autor wpisu lub lider może wykonać te akcje.', 'error')
			return
		}
		...
		const shouldDelete = await AppUtils.confirmDialog({
			title: entryType === 'announcement' ? 'Usuń ważny temat?' : 'Usuń notatkę?',
			message:
				entryType === 'announcement'
					? 'Ta operacja usunie przypięty komunikat z tablicy zespołu.'
					: 'Ta operacja usunie wpis z listy wspólnych notatek.',
		...
		try {
			if (entryType === 'announcement') {
				...
				showFeedbackMessage('Usunięto ważny temat z tablicy.', 'success')
			} else {
				notesService.deleteNote({ noteId: entryId, actor: currentUser })
				if (state.editingNoteId === entryId) closeNoteComposer()
				showFeedbackMessage('Usunięto notatkę ze wspólnej tablicy.', 'success')
			}

			refreshView()
		} catch (error) {
			showFeedbackMessage(error.message || 'Nie udało się usunąć wpisu.', 'error')
		}
	}
	...
	async function handleTaskClick(event) {
		...
		if (!canManageTasks(currentUser)) {
			showFeedbackMessage('Tylko lider może edytować lub usuwać zadania.', 'error')
			return
		}
		...
		const shouldDelete = await AppUtils.confirmDialog({
			title: 'Usuń zadanie?',
			message: 'Ta operacja usunie zadanie z tablicy zespołu.',
		...
		try {
			...
			showFeedbackMessage('Usunięto zadanie z tablicy.', 'success')
			refreshView()
		} catch (error) {
			showFeedbackMessage(error.message || 'Nie udało się usunąć zadania.', 'error')
		}
	}
	...
	function handleTaskStatusChange(event) {
		...
		try {
			...
			showFeedbackMessage(error.message || 'Nie udało się zmienić statusu zadania.', 'error')
			refreshView()
		}
	}
