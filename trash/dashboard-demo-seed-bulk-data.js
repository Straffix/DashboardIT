(function initializeDashboardDemoSeedBulkData() {
	...
	const buildSeedPayload = () => {
		...
		const bookmarks = [
			...
			['bookmark-demo-6', 'demo-user-4', 'Lista salek', 'https://rooms.example.local', 'Konfiguracja urzadzen Teams Rooms i tabletow.'],
		...
		const announcements = [
			['announcement-demo-1', 'Okno serwisowe w piatek', 'Od 18:00 do 19:30 planowany jest restart VPN oraz kontrolerow wydruku. Dzial obslugi ma byc pod telefonem.', 'demo-admin', 24],
			['announcement-demo-2', 'Nowa pula laptopow Dell', 'Do magazynu dotarla nowa partia urzadzen. Prosze zuzywac ja najpierw do onboardingow planowanych na kwiecien i maj.', 'demo-user-1', 22],
			['announcement-demo-3', 'Audyt sal konferencyjnych', 'Do konca miesiaca trzeba potwierdzic wersje Teams Rooms, aktywne mikrofony i dostepnosc zapasowych pilotow.', 'demo-user-4', 20],
			['announcement-demo-4', 'Wydania na start kwartalu', 'Najwieksze obciazenie onboardingowe przypada na kwiecien i maj. Zarezerwujcie czas na przygotowanie monitorow i stacji dokujacych.', 'demo-admin', 18],
		...
		const notes = [
			['note-demo-1', 'Na stanowisku w sali B3 nadal trzeba podmienic zasilacz do monitora konferencyjnego.', 'demo-user-2', 23],
			['note-demo-2', 'HR potwierdzil start onboardingow na kwiecien. W magazynie trzeba odlozyc 4 komplety: laptop, mysz, headset, torba.', 'demo-admin', 24],
			['note-demo-3', 'Po testach drukarki w recepcji trzeba jeszcze wgrac finalny sterownik do stanowiska kierownika zmiany.', 'demo-user-3', 21],
			['note-demo-4', 'Monitoringi domeny z wygasnieciem do 14 dni: sprawdzic serie RU 45120-45160 przed koncem tygodnia.', 'demo-user-1', 20],
			['note-demo-5', 'W sali Zarzad 2 trzeba wymienic baterie w pilocie i sprawdzic drugi display po ostatnim restarcie.', 'demo-user-4', 19],
			['note-demo-6', 'Do zamkniecia pozostaje jeszcze import starej listy wymian z oddzialu Lodz. Brakuje 3 numerow SN.', 'demo-user-5', 18],
			['note-demo-7', 'Prosba od security: nowe onboardingi maja dostawac MFA w pakiecie startowym razem z instrukcja logowania.', 'demo-user-6', 17],
			['note-demo-8', 'Dla majowych wymian warto zarezerwowac dodatkowe monitory 24 cale, bo zapotrzebowanie z retail wzrasta.', 'demo-admin', 16],
		...
		const noteTasks = [
			['task-demo-1', 'Zweryfikuj pule docking station', 'Sprawdz magazyn i oznacz dwie kompletne sztuki pod onboarding z przyszlego tygodnia.', 'demo-user-1', 'demo-admin', 'demo-admin', 'todo', 'high', 23],
			['task-demo-2', 'Domknij wymiane dla Karoliny Banas', 'Zarchiwizuj potwierdzenie odbioru starego urzadzenia i zaktualizuj wpis w zestawieniu HR.', 'demo-user-2', 'demo-admin', 'demo-user-2', 'in_progress', 'medium', 20],
			['task-demo-3', 'Spisz licencje z sal konferencyjnych', 'Potrzebna lista aktualnych wersji Teams Rooms i numerow seryjnych tabletow sterujacych.', 'demo-user-3', 'demo-admin', 'demo-admin', 'done', 'low', 18],
			['task-demo-4', 'Przejrzyj wygasajace domeny', 'Urzadzenia z terminem do 10 dni maja dostac priorytetowe przedluzenie albo decyzje o wymianie.', 'demo-user-4', 'demo-admin', 'demo-user-4', 'in_progress', 'high', 21],
			['task-demo-5', 'Zweryfikuj checklisty kwietniowe', 'Dla wszystkich onboardingow w kwietniu sprawdz gotowosc laptopa, monitora i uprawnien startowych.', 'demo-user-5', 'demo-admin', 'demo-admin', 'todo', 'high', 19],
			['task-demo-6', 'Odswiez grafike sali zarzadu', 'Po wymianie kontrolera trzeba zrobic nowy zestaw screenow i instrukcje dla asystentek.', 'demo-user-6', 'demo-user-4', 'demo-user-6', 'todo', 'medium', 18],
			['task-demo-7', 'Zamknij incydent drukarki retail 12', 'Potwierdz stabilnosc po wymianie modulu i dopisz notatke do historii urzadzenia.', 'demo-user-3', 'demo-user-5', 'demo-user-3', 'done', 'medium', 17],
			['task-demo-8', 'Przygotuj raport miesieczny', 'Potrzebne podsumowanie onboardingow, wymian i urzadzen poza domena za marzec 2026.', 'demo-user-8', 'demo-admin', 'demo-admin', 'todo', 'high', 16],
		...
		const plannerTasks = [
			['planner-demo-1', 'Przeglad ticketow VIP', '2026-03-25', '09:15', 'high', 'Szybkie sprawdzenie ticketow z eskalacji i priorytetow CFO.'],
			['planner-demo-2', 'Przekazanie laptopa dla Marty', '2026-03-25', '12:45', 'medium', 'Onboarding room 2, przygotowac plecak, zasilacz i login startowy.'],
			['planner-demo-3', 'Kontrola monitoringu domeny', '2026-03-26', '08:30', 'low', 'Zweryfikowac urzadzenia z terminem wygasniecia do 14 dni.'],
			['planner-demo-4', 'Import starej bazy wymian', '2026-03-26', '10:00', 'medium', 'Scalic dane z oddzialu Lodz i uzupelnic brakujace SN.'],
			['planner-demo-5', 'Weryfikacja salek A i B', '2026-03-27', '09:30', 'high', 'Sprawdzic display, mikrofony i komplet kabli po porannych spotkaniach.'],
			['planner-demo-6', 'Przeglad magazynu akcesoriow', '2026-03-27', '13:15', 'low', 'Policzyc myszki, klawiatury i headsety pod onboardingi kwietniowe.'],
			['planner-demo-7', 'Status call z HR', '2026-03-28', '11:00', 'medium', 'Potwierdzenie listy startow na kwiecien i maj.'],
			['planner-demo-8', 'Czyszczenie zwrotow', '2026-03-28', '14:20', 'low', 'Przygotowac 3 urzadzenia do ponownego wdrozenia.'],
			['planner-demo-9', 'Przeglad wydan na kwiecien', '2026-04-02', '09:10', 'high', 'Skontrolowac gotowosc wszystkich wpisow z pierwszego tygodnia kwietnia.'],
			['planner-demo-10', 'Raport miesieczny', '2026-04-03', '15:00', 'medium', 'Domknac zestawienie za marzec dla lidera i HR.'],
			['planner-demo-11', 'Przygotowanie sali zarzadu', '2026-04-07', '08:45', 'high', 'Test audio, tabletu sterujacego i zapasowego HDMI.'],
			['planner-demo-12', 'Przekazanie 2 laptopow z magazynu', '2026-04-08', '12:10', 'medium', 'Wydania dla retail po finalnym sprawdzeniu obrazow systemowych.'],
